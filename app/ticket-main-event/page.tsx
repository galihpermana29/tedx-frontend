...
const formData: FormFieldI[] = [
  ...
  {
    name: 'disabilitas',
    label: 'Apakah kamu seseorang yang berkebutuhan khusus (Disabilitas)?',
    type: 'radio',
    placeholder: 'Ex: Iya',
    data: [
      {
...
export default function TicketMainEvent() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await WebsiteAPI.createTransactionMainEvent(payload);
      toast.success(
                      ...
                      <div className="flex flex-col h-auto">
                        ...
                        <div
                          className={`flex gap-[10px] mt-[10px] ${
                            d.name === 'disabilitas' ? 'flex-col' : 'flex-row'
                          }`}>
                          {d.data.map((item) => (
                        ...
                        <div className="mt-[10px] text-red-400">
                          *Notes: Apabila membawa pendamping, pendamping
                          tersebut diharuskan untuk membeli tiket.
                        </div>
                      </div>
                    ) : d.type === 'increement' ? (
